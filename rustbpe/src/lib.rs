// Copying this rust portion straight from nanochat
//
// SUMMARY
// -------
// This file implements a high-performance Byte Pair Encoding (BPE) tokenizer trainer in Rust,
// designed to be called from Python via PyO3. It is significantly faster than pure Python implementations
// because it leverages parallel processing (rayon) and efficient memory management.
//
// MAIN COMPONENTS
// ---------------
// 1. struct Tokenizer (#[pyclass]):
//    - The main Python class wrapper.
//    - Holds the learned merges (HashMap<Pair, u32>) and the regex pattern.
//    - Exposes methods like `train_from_iterator` and `encode` to Python.
//
// 2. struct Word:
//    - Represents a single tokenized word as a sequence of token IDs.
//    - `merge_pair`: The critical inner loop function. It merges a specific pair (e.g., 't', 'h' -> 'th')
//       within the word and calculates the count deltas (+1/-1) to update global stats efficiently.
//
// 3. count_pairs_parallel:
//    - Uses Rayon to count pair frequencies across all words in parallel.
//    - This is the "map-reduce" step that speeds up the initial counting phase.
//
// 4. train_core_incremental:
//    - The core BPE training loop.
//    - Uses a Max-Heap to always pick the most frequent pair to merge.
//    - Merges incrementally: instead of recounting the whole dataset after every merge,
//      it only updates the counts for words that were actually modified.
//
// KEY TAKEAWAYS
// -------------
// - Performance comes from minimizing global passes over data. We only scan the full dataset once per merge round.
// - Concurrency is handled by Rust's `rayon` crate, which splits work across CPU cores automatically.
// - The GIL (Global Interpreter Lock) is released during heavy computation so Python remains responsive.
